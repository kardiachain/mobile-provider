...
const INPAGE = 'kardia-inpage';
const CONTENT_SCRIPT = 'kardia-contentscript';
const PROVIDER = 'kardia-provider';

// Setup stream for content script communication
const kardiaStream = new ReactNativePostMessageStream({
  name: INPAGE,
  target: CONTENT_SCRIPT,
});

// Initialize provider object (window.kardiachain)
initializeProvider({
  connectionStream: kardiaStream,
  shouldSendMetadata: false,
});

// Set content script post-setup function
Object.defineProperty(window, '_kardiaSetupProvider', {
  value: () => {
    setupProviderStreams();
    delete window._kardiaSetupProvider;
  },
  configurable: true,
  enumerable: false,
  writable: false,
...
function setupProviderStreams() {
  ...
  pump(pageMux, pageStream, pageMux, (err) =>
    logStreamDisconnectWarning('KardiaChain Inpage Multiplex', err),
  );
  pump(appMux, appStream, appMux, (err) => {
    logStreamDisconnectWarning('KardiaChain Background Multiplex', err);
...
function forwardTrafficBetweenMuxes(channelName, muxA, muxB) {
  ...
  pump(channelA, channelB, channelA, (err) =>
    logStreamDisconnectWarning(
      `KardiaChain muxed traffic for channel "${channelName}" failed.`,
      err,
    ),
  );
}
...
function logStreamDisconnectWarning(remoteLabel, err) {
  let warningMsg = `KardiaChainContentscript - lost connection to ${remoteLabel}`;
  if (err) {
    warningMsg += `\n${err.stack}`;
  }
